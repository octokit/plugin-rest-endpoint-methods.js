...
name: Create a budget for an organization
example: octokit.rest.billing.createOrganizationBudget({ org })
...
```js
octokit.rest.billing.createOrganizationBudget({
  org,
});
```
...
See also: [GitHub Developer Guide documentation](https://docs.github.com/rest/billing/budgets#create-a-budget-for-an-organization).
