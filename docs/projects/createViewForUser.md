...
name: Create a view for a user-owned project
example: octokit.rest.projects.createViewForUser({ user_id, project_number, name, layout })
route: POST /users/{user_id}/projectsV2/{project_number}/views
scope: projects
type: API method
---
...
```js
octokit.rest.projects.createViewForUser({
  user_id,
  project_number,
  name,
  layout,
...
See also: [GitHub Developer Guide documentation](https://docs.github.com/rest/projects/views#create-a-view-for-a-user-owned-project).
