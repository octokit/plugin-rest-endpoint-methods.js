...
name: Get a task by repo
example: octokit.rest.agentTasks.getTaskByRepoAndId({ owner, repo, task_id })
...
```js
octokit.rest.agentTasks.getTaskByRepoAndId({
  owner,
  repo,
  task_id,
});
...
See also: [GitHub Developer Guide documentation](https://docs.github.com/rest/agent-tasks/agent-tasks#get-a-task-by-repo).
