...
name: Add issue field values to an issue
example: octokit.rest.issues.addIssueFieldValues({ owner, repo, issue_number, issue_field_values[].field_id, issue_field_values[].value })
route: POST /repos/{owner}/{repo}/issues/{issue_number}/issue-field-values
scope: issues
type: API method
---
...
```js
octokit.rest.issues.addIssueFieldValues({
        owner,
repo,
issue_number,
...
See also: [GitHub Developer Guide documentation](https://docs.github.com/rest/issues/issue-field-values#add-issue-field-values-to-an-issue).
